...
def read_gradient(file, adata, patch_size, bin_size, dia_estimate, segmentation_labels= 'watershed_labels'):
    watershed2x = {}
    watershed2y = {}
    for i in tqdm(list(range(adata.layers[segmentation_labels].shape[0]))):
        for j in range(adata.layers[segmentation_labels].shape[1]):
            if adata.layers[segmentation_labels][i, j] == 0:
                continue
            if adata.layers[segmentation_labels][i, j] in watershed2x:
                watershed2x[adata.layers[segmentation_labels][i, j]].append(i)
                watershed2y[adata.layers[segmentation_labels][i, j]].append(j)
            else:
                watershed2x[adata.layers[segmentation_labels][i, j]] = [i]
                watershed2y[adata.layers[segmentation_labels][i, j]] = [j]
    ...
    with open(file) as fr:
        header = fr.readline()
        print(header)
        #assert header == 'x y b class_logit\n'
        ...
        for line in tqdm(fr):
            ...
            for center in watershed2center:
                ...
                if dis_c <= int(dia_estimate):
                    class_prior += dir_to_class(watershed2center[center][0] - int(x) , watershed2center[center][1] - int(y),
                                                class_num, watershed2center[center][0], watershed2center[center][1])
                if dis_c <= int(dia_estimate) * 1.25:
                    class_prior_ext += dir_to_class(watershed2center[center][0] - int(x) , watershed2center[center][1] - int(y),
                                                    class_num, watershed2center[center][0], watershed2center[center][1])
    ...
    print('max x y', np.max(list_x), np.max(list_y))

    def diffuse(bin_size):
        dx_new = dx.copy()
        dy_new = dy.copy()
        intensity_new = intensity.copy()
        for i in tqdm(list(range(dx_new.shape[0]))):
            if i % bin_size == 0:
...
def remove_small_cells(label_mat, cell_size_threshold=200):
    label_size = {}
    for i in range(label_mat.shape[0]):
        for j in range(label_mat.shape[1]):
            if label_mat[i , j] > 0:
    ...
    for i in range(label_mat.shape[0]):
        for j in range(label_mat.shape[1]):
            if label_mat[i, j] > 0 and label_size[label_mat[i, j]] >= cell_size_threshold:
...
def result_stats(merged, patch_id, results_folder):
    cell2nspots = {}
    ...
    with open(results_folder / f'cell_stats_{patch_id}.txt', 'w') as fw:
...
def postprocess(startx, starty, patchsize, bin_size, dia_estimate,
                save_folder, segmentation_labels, cell_size_threshold=200):
    ...
    patch_id =  startx + ':' + starty + ':' + patchsize + ':' + patchsize
    adata = ad.read_h5ad(data_folder / f'spots{patch_id}.h5ad')
    patchsizex = adata.X.shape[0]
    patchsizey = adata.X.shape[1]

    np.save(results_folder / f'segmentation_labels_{patch_id}', adata.layers[segmentation_labels])
    tifffile.imwrite(results_folder / f'segmentation_labels_{patch_id}.tif', adata.layers[segmentation_labels].astype(np.uint16))

    print('Adjust spot prediction priors...')

    intensity, dx, dy, pred_U, pred_V, pred_C = read_gradient(results_folder / f'spot_prediction_{patch_id}.txt', adata, (int(patchsizex), int(patchsizey)), bin_size, dia_estimate, segmentation_labels)

    np.save(results_folder / f'intensity_{patch_id}', intensity)
    np.save(results_folder / f'dx_{patch_id}', dx)
    np.save(results_folder / f'dy_{patch_id}', dy)
    np.save(results_folder / f'pred_U_{patch_id}', pred_U)
    np.save(results_folder / f'pred_V_{patch_id}', pred_V)
    print('Gradient flow tracking...')

    Segmentation, Sinks, dx, dy, mask = gvf_tracking(dx, dy, intensity, bin_size)

    np.save(results_folder / f'spot2cell_{patch_id}', Segmentation)
    np.save(results_folder / f'Sinks{patch_id}',Sinks)
    np.save(results_folder / f'dx_{patch_id}', dx)
    np.save(results_folder / f'dy_{patch_id}', dy)
    np.save(results_folder / f'mask_{patch_id}', mask)
                ...
                for m in range(int(downrs)):
                    for n in range(int(downrs)):
                        if i + m < int(patchsizex) and j + n < int(patchsizey):
                            ...
                            expand[i + m, j + n] = 1
    merged = remove_small_cells(merged, cell_size_threshold)
    ...
    fw = open(results_folder / f'spot2cell_{patch_id}.txt', 'w')
    for i in range(merged.shape[0]):
        for j in range(merged.shape[1]):
            if merged[i, j] > 0:
    ...
    nucl_labels = adata.layers[segmentation_labels]
    fw = open(results_folder / f'spot2nucl_{patch_id}.txt', 'w')
    ...
    result_stats(merged, patch_id, results_folder)
    np.save(results_folder / f'cell_masks_{patch_id}', merged)
    np.save(results_folder / f'edges_{patch_id}', edges)
    tifffile.imwrite(results_folder / f'cell_masks_{patch_id}.tif', merged.astype(np.uint16))
    tifffile.imwrite(results_folder / f'edges_{patch_id}.tif', edges.astype(np.uint16))
    ...
    plt.savefig(results_folder / f'cell_masks_{patch_id}.png')
    np.save(results_folder / f'cell_masks_{patch_id}_after_modulo.tif', merged.astype(np.uint16))
    tifffile.imwrite(results_folder / f'cell_masks_{patch_id}_after_modulo.tif', merged.astype(np.uint16))
...
if __name__=="__main__":
    ...
    save_folder = "/cluster/CBIO/data1/ablondel1/vallot/202304141840_1184_VallotAB5301andAG5988_VMSC09402/region_0/crop10000_2_2/scs_pred/"

    postprocess(0, 0, patch_size, bin_size, dia_estimate,
